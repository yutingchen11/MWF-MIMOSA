%% Plot complex NRMSE for different loss settings ¡ª Nature style (no L1, % unit)
clear; clc; close all;
...
xticklabels_short = {'WRCL','WCL','CL','RCL','L2'};

% ---------------- Convert to percentage ----------------
complex_nrse_pct = 100 * complex_nrse;

% ---------------- Sort ascending ----------------
[complex_nrse_sorted, idx] = sort(complex_nrse_pct, 'ascend');
cfg_name_sorted    = cfg_name(idx);
xticklabels_sorted = xticklabels_short(idx);

% ---------------- Nature-palette category colors ----------------
color_full   = [0.173, 0.482, 0.714];   % deep steel blue   (full loss)
color_relcpx = [0.404, 0.663, 0.812];   % light steel blue  (other relCplx)
color_plain  = [0.600, 0.780, 0.620];   % muted sage green  (plain L2)
...
for i = 1:numel(cfg_name_sorted)
    ...
    elseif contains(name_i, 'plainL2')
...
% yOffset = max(0.03 * yRange, 0.005);

% for i = 1:numel(complex_nrse_sorted)
%     text(i, complex_nrse_sorted(i) + yOffset, ...
%         sprintf('%.2f%%', complex_nrse_sorted(i)), ...
...
lgd = legend([h1 h2 h3], ...
    {'Full loss', 'Other relCplx', 'Plain L2'}, ...
    'Location', 'northeast', ...
    'FontName', 'Arial', ...
    'FontSize', 6, ...
    'Box',      'off');
...
% ---------------- Export ----------------
outDir = 'fig_loss_nrmse_nature_noL1';
if ~exist(outDir,'dir'), mkdir(outDir); end

exportgraphics(fig, fullfile(outDir,'Fig_loss_nrmse_nature_noL1_pct.png'), 'Resolution', 300);
exportgraphics(fig, fullfile(outDir,'Fig_loss_nrmse_nature_noL1_pct.pdf'), 'ContentType', 'vector');