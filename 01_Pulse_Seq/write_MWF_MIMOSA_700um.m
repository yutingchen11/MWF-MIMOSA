...
%%
traj = readmatrix('csv_imports/cplm/fov_224x192_msize_320x274_tl_119_ncal_0_spiral_30_acc_2.6x2.6_nTR430_nph5.txt');
...
fov=[200 224 192]*1e-3;         % Define FOV and resolution  
N = [286 320 274];
...
seq.write(['MWF_MIMOSA_700um','.seq']);

warning('OFF', 'mr:restoreShape')
seq.calcPNS('MP_GPA_K2309_2250V_951A_AS82.asc');
...
for iY = 1:nETL
    ...
    % index = (iY-1)*nacq+ ind_acq + (iZ-1)*nETL*nacq;
%     index = iY+ (ind_acq-1)*nETL + (iZ-1)*nETL*nacq;
...
%     disp(['acq_' num2str(ind_acq) ',index=',num2str(index)]);
    ...
    % figure(ind_acq);hold on;scatter(traj(index,2),traj(index,3));
...
for iY = 1:nETL
    ...
    for c=1:length(TEs) % loop over TEs
        ...
        %###########
        % Calculate index for ky-kz look up table
        % ind_acq_me = ind_acq + c - 1 ;% complememt
        ind_acq_me = ind_acq;
        % index = (iY-1)*nacq+ ind_acq_me + (iZ-1)*nETL*nacq;
%         index = iY+ (ind_acq_me-1)*nETL + (iZ-1)*nETL*nacq;
  % v2:
%         index = iY+ (ind_acq_me-1)*nTR*nETL + (iZ-1)*nETL;% $ cplm
        % outc
...
%         figure(4);hold on;scatter(traj(index,2),traj(index,3),10,ind_acq_me);
        ...
        if c==1 
            ...
            % seq.addBlock(mr.scaleGrad(gyReph,-(stepsY(index) - stepsY(ind_pre))), ...
            %     mr.scaleGrad(gzReph,-(stepsZ(index)- stepsZ(ind_pre))));    % Gz, Gy blips, Gx pre-winder
