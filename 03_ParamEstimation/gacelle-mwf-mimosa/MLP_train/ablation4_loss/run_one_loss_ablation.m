function run_one_loss_ablation(cfg_name, seed)
% English comments only
%
% Minimal-change loss ablation based on the old verified run_one_ablation.m
...
%
% Usage:
%   run_one_loss_ablation('LOSS01__relCplx__W1__N1')
%   run_one_loss_ablation(1)
%   run_one_loss_ablation('LOSS05__plainL1', 0)
...
cfgs = build_loss_ablation_configs();
cfg_id = resolve_cfg_id_loss(cfg_name, cfgs);
cfg = cfgs(cfg_id);
...
root_prefix = 'MIMOSA_lossAbl_minChange_v1';
root_dir = fullfile(pwd, root_prefix);
if ~exist(root_dir, 'dir')
    mkdir(root_dir);
end

run_name = sprintf('L%02d_%s_F%02d_seed%d', cfg_id, sanitize_name(cfg.name), nfeatures, seed);
...
fprintf('\n============================================================\n');
fprintf('Running loss ablation: %s\n', cfg.name);
fprintf('Config ID       : %d\n', cfg_id);
fprintf('Seed            : %d\n', seed);
...
accfun = dlaccelerate(@modelGradients_mlp_MIMOSA_lossSwitch);
...
for epoch = 1:Nepochs
    ...
    for it = 1:itersPerEpoch
        ...
        [gradients, loss, loss_norm] = dlfeval( ...
            ...
            cfg.loss_mode, cfg.use_weight, cfg.use_norm, cfg.epsA, cfg.tau); %#ok<NASGU>
...
write_metrics_text_loss(fullfile(out_dir, [run_name '_summary.txt']), ...
...
function cfgs = build_loss_ablation_configs()
% English comments only
% Fixed verified baseline:
%   freqRaw + PT+prepOH + FW0
%   feature_idx = [1:11 16:20]
% Fixed network width is handled in the main function.
...
cfgs = struct('name', {}, 'feature_idx', {}, 'loss_name', {}, ...
              'loss_mode', {}, 'use_weight', {}, 'use_norm', {}, ...
              'epsA', {}, 'tau', {});

defs = { ...
    'LOSS01__relCplx__W1__N1', 'relCplx', 1, 1, 1; ...
    'LOSS02__relCplx__W1__N0', 'relCplx', 1, 1, 0; ...
    'LOSS03__relCplx__W0__N1', 'relCplx', 1, 0, 1; ...
    'LOSS04__relCplx__W0__N0', 'relCplx', 1, 0, 0; ...
    'LOSS05__plainL1',         'plainL1', 2, 0, 0; ...
    'LOSS06__plainL2',         'plainL2', 3, 0, 0; ...
    };

for k = 1:size(defs, 1)
    cfgs(k).name        = defs{k,1}; %#ok<AGROW>
    cfgs(k).loss_name   = defs{k,2}; %#ok<AGROW>
    cfgs(k).loss_mode   = defs{k,3}; %#ok<AGROW>
    cfgs(k).use_weight  = defs{k,4}; %#ok<AGROW>
    cfgs(k).use_norm    = defs{k,5}; %#ok<AGROW>
...
function [gradients, loss, loss_norm] = modelGradients_mlp_MIMOSA_lossSwitch( ...
...
% English comment: build the full 22-channel tensor first, then subset it
nFull = 22;
