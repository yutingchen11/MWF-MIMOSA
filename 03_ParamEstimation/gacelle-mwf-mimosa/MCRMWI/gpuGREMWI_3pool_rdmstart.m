classdef gpuGREMWI_3pool_rdmstart < handle
% Kwok-Shing Chan @ MGH
...
% Date modified: 22 September 2024
    ...
    properties
        ...
        modelParams     = { 'S0';   'MWF';  'IEW';  'R2sMW';'R2sIEW'; 'freqMW';'freqIEW';'dfreqBKG';'dpini'};
        ub              = [    1;     0.3;      1;      200;     50;       0.25;    0.05;       0.4;   pi/2];
        lb              = [ 1e-8;    1e-8;   1e-8;       50;      2;       -0.05;    -0.1;      -0.4;  -pi/2];
    ...
    methods
        ...
        % constructuor
        function this = gpuGREMWI_3pool_rdmstart(te,fixed_params)
        ...
        function this = updateProperty(this, fitting)
            ...
            % if fitting.DIMWI.isFitR2sFW == 0
            %     idx = find(ismember(this.modelParams,'R2sFW'));
            %     this.modelParams(idx)    = [];
            %     this.lb(idx)              = [];
            %     this.ub(idx)              = [];
            %     this.startPoint(idx)      = [];
        ...
        %   .DIMWI.isFitFreqIEW  : IEW frequency is a free parameter, default = true
        %   .DIMWI.isFitR2sFW   : FW R2* is a free parameter, default = true
        % 
        % Output
        % -----------
            ...
            if ischar(fitting.start)
                switch lower(fitting.start)
                    case 'prior'
                        ...
                        % x0.IFW = extraData.IEW;
                        x0.S0 = extraData.PD;
    
                    case 'default'
                        % randomize 
                        % use fixed points with random MWF initial value
                        % startPoint_local = this.startPoint;
                        % 
                        % idx_mwf = find(ismember(this.modelParams,'MWF'));
                        % if ~isempty(idx_mwf)
                        %     % English comment: random MWF start point in [0.05, 0.15]
                        %     startPoint_local(idx_mwf) = 0.05 + (0.15 - 0.05) * rand;
                        % end
                        % 
                        % fprintf('Using default starting points for all voxels at [%s]: [%s]\n', ...
                        %     cell2str(this.modelParams), ...
                        %     replace(num2str(startPoint_local(:).',' %.4f'),' ',','));
        ...
        % Estimation starting points 
            ...
            idx = find(ismember(this.modelParams,'R2sFW'));
            if ~isempty(idx)
                % if R2*FW is a free parameter then set it
                R2sFW = R2s + 3;
                R2sFW(isnan(R2sFW)) = single(this.startPoint(idx)); R2sFW(isinf(R2sFW)) = single(this.startPoint(idx)); 
                R2sFW(R2sFW < this.lb(idx)) = single(this.lb(idx)); R2sFW(R2sFW > this.ub(idx)) = single(this.ub(idx));
                pars0.R2sFW = single(R2sFW);
            % else
            %     % if R2*FW is not a free parameter then reset R2*FW
            %     idx = find(ismember(this.modelParams,'R2sIEW'));
        ...
        function [s] = FWD(this, pars, fitting, extraData)
            ...
            S0   = pars.S0;
            mwf  = pars.MWF;
            if fitting.DIMWI.isFitIEW; iwf  = pars.IEW; else; iwf = extraData.IEW; end
            r2sMW   = pars.R2sMW;
            r2sIEW   = pars.R2sIEW;

            % if fitting.DIMWI.isFitR2sFW;    r2sFW   = pars.R2sFW;   end
            ...
            % if use HCFM to derive either freqMW|freqIEW|R2*FW, then computee g-ratio
            if ~fitting.DIMWI.isFitFreqMW || ~fitting.DIMWI.isFitFreqIEW || ~fitting.DIMWI.isFitR2sFW
                hcfm_obj = HCFM(this.te,this.B0);

                % g-ratio 
                g = hcfm_obj.gratio(abs(S0IEW),abs(S0MW)/this.rho_mw);

            end
            
            % extra decay on extracellular water estimated by HCFM 
            % if ~fitting.DIMWI.isFitR2sFW
            % 
            %     % assume extracellular water has the same T2* as intra-axonal water
            %     % r2sFW   = r2sIEW;
            %     % fibre volume fraction
            %     fvf     = hcfm_obj.FibreVolumeFraction(abs(S0IEW),abs(S0FW),abs(S0MW)/this.rho_mw);
            % 
            %     % signal dephase in extracellular water due to myelin sheath, Eq.[A7]
            %     decayFW = hcfm_obj.DephasingExtraaxonal(fvf,g,this.x_i,this.x_a,extraData.theta);
            % 
            % else
            %     decayFW = 0;
            % end
            ...
            Sreal = sum((   S0MW .* exp(-TE .* r2sMW) .* cos(TE .* 2.*pi.*(freqMW+freqBKG).*this.B0.*this.gyro + pini) + ...
                            S0IEW .* exp(-TE .* r2sIEW) .* cos(TE .* 2.*pi.*(freqIEW+freqBKG).*this.B0.*this.gyro + pini) + ...
                            S0FW .* exp(-TE .* r2sFW) .* cos(TE .* 2.*pi.*(freqFW+freqBKG).*this.B0.*this.gyro + pini) .* exp(-decayFW) ).*extraData.ff,5);

            Simag = sum((   S0MW .* exp(-TE .* r2sMW) .* sin(TE .* 2.*pi.*(freqMW+freqBKG).*this.B0.*this.gyro + pini) + ...
                            S0IEW .* exp(-TE .* r2sIEW) .* sin(TE .* 2.*pi.*(freqIEW+freqBKG).*this.B0.*this.gyro + pini) + ...
                            S0FW .* exp(-TE .* r2sFW) .* sin(TE .* 2.*pi.*(freqFW+freqBKG).*this.B0.*this.gyro + pini) .* exp(-decayFW) ).*extraData.ff,5);
    ...
    methods(Static)
        ...
        % t2s           : T2* of the two pools, in second, [T2sMW,T2sIFW], if empty
        ...
        %
            ...
            % check input
            if isempty(t2s)
                t2s = [10e-3, 60e-3];   % 3T, [MW, IFW], in second
            end
        ...
        function fitting2 = check_set_default(fitting,data)
            ...
            if ~isfield(fitting,'DIMWI') || ~isfield(fitting.DIMWI,'isFitR2sFW');   fitting2.DIMWI.isFitR2sFW   = true; end
