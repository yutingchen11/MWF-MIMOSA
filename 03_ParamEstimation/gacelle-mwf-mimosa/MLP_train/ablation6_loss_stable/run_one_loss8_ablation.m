function run_one_loss8_ablation(cfg_name, seed)
% English comments only
%
% Minimal-change 8-loss ablation based on the verified baseline code.
% Baseline is fixed to:
%   freqRaw + PT+prepOH + FW0
%   feature_idx = [1:11 16:20]
%   numNeurons  = base / 2
%
% Loss family:
%   1. WRCL = weighted + relative + Charbonnier complex loss
%   2. WCL  = weighted + Charbonnier complex loss
%   3. RCL  = relative + Charbonnier complex loss
%   4. CL   = plain Charbonnier complex loss
%   5. WRL1 = weighted + relative + L1 residual loss
%   6. WL1  = weighted + L1 residual loss
%   7. RL1  = relative + L1 residual loss
%   8. L1   = plain L1 residual loss
%
% Usage:
%   run_one_loss8_ablation('LOSS01__WRCL')
%   run_one_loss8_ablation(1)
%   run_one_loss8_ablation('LOSS08__L1', 0)
...
cfgs = build_loss8_ablation_configs();
cfg_id = resolve_cfg_id_loss8(cfg_name, cfgs);
cfg = cfgs(cfg_id);
...
root_prefix = 'MIMOSA_loss8Abl_minChange_v1';
root_dir = fullfile(pwd, root_prefix);
if ~exist(root_dir, 'dir')
    mkdir(root_dir);
end

run_name = sprintf('L%02d_%s_F%02d_seed%d', ...
    cfg_id, sanitize_name(cfg.name), nfeatures, seed);
...
fprintf('\n============================================================\n');
fprintf('Running 8-loss ablation: %s\n', cfg.name);
fprintf('Config ID       : %d\n', cfg_id);
fprintf('Seed            : %d\n', seed);
fprintf('Feature count   : %d\n', nfeatures);
fprintf('Feature idx     : %s\n', mat2str(feature_idx));
fprintf('Loss abbr       : %s\n', cfg.loss_abbr);
fprintf('Residual type   : %s\n', cfg.residual_name);
fprintf('Use weight      : %d\n', cfg.use_weight);
fprintf('Use norm        : %d\n', cfg.use_norm);
fprintf('tau             : %.8g\n', cfg.tau);
fprintf('epsA            : %.8g\n', cfg.epsA);
...
R2p_MW_range_Hz  = [ ...
    max(0, R2s_MW_cap_range_Hz(1)  - max(R2_MW_range_Hz)), ...
    max(0, R2s_MW_cap_range_Hz(2)  - min(R2_MW_range_Hz)) ];

R2p_IEW_range_Hz = [ ...
    max(0, R2s_IEW_cap_range_Hz(1) - max(R2_IEW_range_Hz)), ...
...
accfun = dlaccelerate(@modelGradients_mlp_MIMOSA_loss8Switch);
...
for epoch = 1:Nepochs
    ...
    for it = 1:itersPerEpoch
        ...
        [gradients, loss, loss_norm] = dlfeval( ...
            accfun, parameters, dlin, dlout, alpha_leaky, ...
            cfg.residual_mode, cfg.use_weight, cfg.use_norm, cfg.epsA, cfg.tau); %#ok<NASGU>
    ...
    if saveCheckpoints && mod(epoch, checkpointEvery) == 0
        dlnet_ckpt = struct();
        dlnet_ckpt.parameters = parameters;
        dlnet_ckpt.epoch      = epoch;
        dlnet_ckpt.alpha      = alpha_leaky;

        save(fullfile(out_dir, sprintf('%s_epoch_%d.mat', run_name, epoch)), ...
            'dlnet_ckpt', 'epoch_loss_mean', 'cfg', 'feature_idx', 'seed');
    end
end
...
save(fullfile(out_dir, [run_name '_final.mat']), ...
    'dlnet', 'epoch_loss_mean', 'valMetrics', 'valPerPoint', ...
    'cfg', 'feature_idx', 'seed');

write_metrics_text_loss8(fullfile(out_dir, [run_name '_summary.txt']), ...
...
function cfg_id = resolve_cfg_id_loss8(cfg_name, cfgs)
% English comments only
...
function write_metrics_text_loss8(txtFile, run_name, cfg, feature_idx, final_train_loss, valMetrics)
% English comments only
...
fprintf(fid, 'Loss abbr: %s\n', cfg.loss_abbr);
fprintf(fid, 'Residual type: %s\n', cfg.residual_name);
...
function cfgs = build_loss8_ablation_configs()
% English comments only
% Fixed verified baseline:
%   freqRaw + PT+prepOH + FW0
%   feature_idx = [1:11 16:20]
% Fixed network width:
%   base / 2
...
% English comment:
% residual_mode = 1 -> Charbonnier residual magnitude
% residual_mode = 2 -> L1 residual sum
defs = { ...
...
function [gradients, loss, loss_norm] = modelGradients_mlp_MIMOSA_loss8Switch( ...
    parameters, dlXf, dlRf, alpha, residual_mode, use_weight, use_norm, epsA0, tau0)
% English comments only
...
switch residual_mode
    case 1
        base_res = sqrt(dPr.^2 + dPi.^2 + epsA);   % Charbonnier residual magnitude
    case 2
        base_res = abs(dPr) + abs(dPi);            % L1 residual sum
    otherwise
        error('Unknown residual_mode: %d', residual_mode);
end

if use_norm
    core = base_res ./ (mag_gt + epsA);
else
    core = base_res;
end
