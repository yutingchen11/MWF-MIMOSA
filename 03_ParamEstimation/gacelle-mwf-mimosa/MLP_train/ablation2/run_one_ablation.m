function run_one_ablation(cfg_name)
% 
%
% Single-ablation training + validation entry point.
% This function is intended for cluster usage, where each job runs one
% ablation config identified by cfg_name.
%
% Usage examples:
%   run_one_ablation('freqRaw__PT__FW0')
%   run_one_ablation("freqRaw+phaseTrig__PT+prepOH__FW1")
%   run_one_ablation(1)
...
root_prefix = 'MIMOSA_featAbl_loss1_tau0p01_v1';
root_dir = fullfile(pwd, root_prefix);
if ~exist(root_dir, 'dir')
    mkdir(root_dir);
end

run_name = sprintf('A%02d_%s_F%02d', cfg_id, sanitize_name(cfg.name), nfeatures);
out_dir  = fullfile(root_dir, run_name);
if ~exist(out_dir, 'dir')
    mkdir(out_dir);
end

save(fullfile(out_dir, [run_name '_config.mat']), 'cfg', 'cfg_id', 'feature_idx');
...
for epoch = 1:Nepochs
    ...
    if saveCheckpoints && mod(epoch, checkpointEvery) == 0
        ...
        save(fullfile(out_dir, sprintf('%s_epoch_%d.mat', run_name, epoch)), ...
            'dlnet_ckpt', 'epoch_loss_mean', 'cfg', 'feature_idx');
    end
end
...
save(fullfile(out_dir, [run_name '_final.mat']), ...
    'dlnet', 'epoch_loss_mean', 'valMetrics', 'valPerPoint', 'cfg', 'feature_idx');
...
for ifr = 1:numel(freqSets)
    for it = 1:numel(timeSets)
        for useFW = 0:1
            ...
            cfgs(ic).freq_mode = freqNames{ifr}; 
            cfgs(ic).time_mode = timeSetNames{it}; 
            cfgs(ic).useFW     = logical(useFW);
            cfgs(ic).feature_idx = feat; 
            cfgs(ic).name = sprintf('%s__%s__FW%d', ...
                cfgs(ic).freq_mode, cfgs(ic).time_mode, useFW); 
        end
    end
end
end
...
% : build the full 22-channel tensor first, then subset it
nFull = 22;
