function run_one_tau_eps_ablation(cfg_name, seed)
% English comments only
%
% Minimal-change tau/eps ablation based on the verified baseline code.
% Baseline is fixed to:
%   freqRaw + PT+prepOH + FW0
%   feature_idx = [1:11 16:20]
%   numNeurons  = base / 2
%   full loss   = relCplx + weight + normalization
%
% Usage:
%   run_one_tau_eps_ablation('TAUEPS01__tau_1em03__eps_1em06')
%   run_one_tau_eps_ablation(1)
%   run_one_tau_eps_ablation('TAUEPS11__tau_1em02__eps_1em04', 0)
...
cfgs = build_tau_eps_ablation_configs();
cfg_id = resolve_cfg_id_tau_eps(cfg_name, cfgs);
cfg = cfgs(cfg_id);
...
root_prefix = 'MIMOSA_tauEpsAbl_fullLoss_v1';
root_dir = fullfile(pwd, root_prefix);
if ~exist(root_dir, 'dir')
    mkdir(root_dir);
end

run_name = sprintf('TE%02d_%s_F%02d_seed%d', ...
...
fprintf('\n============================================================\n');
fprintf('Running tau/eps ablation: %s\n', cfg.name);
fprintf('Config ID       : %d\n', cfg_id);
fprintf('Seed            : %d\n', seed);
...
write_metrics_text_tau_eps(fullfile(out_dir, [run_name '_summary.txt']), ...
...
function cfg_id = resolve_cfg_id_tau_eps(cfg_name, cfgs)
% English comments only
...
function write_metrics_text_tau_eps(txtFile, run_name, cfg, feature_idx, final_train_loss, valMetrics)
% English comments only
...
function cfgs = build_tau_eps_ablation_configs()
% English comments only
% Fixed verified baseline:
%   freqRaw + PT+prepOH + FW0
%   feature_idx = [1:11 16:20]
% Fixed full loss:
%   relCplx + weight + normalization
% Ablation only on tau and epsA
...
for itau = 1:numel(tauList)
    for ieps = 1:numel(epsList)
        ...
        cfgs(ic).name = sprintf('TAUEPS%02d__tau_%s__eps_%s', ...
            ic, val_to_tag(tauVal), val_to_tag(epsVal)); %#ok<AGROW>
    end
end
end

function tag = val_to_tag(x)
% English comments only
