function [Mz_mtx, Mxy_mtx] = sim_mwf_MIMOSA_bp(TR, alpha_deg, esp, turbo_factor, t1_vals, t2_vals, num_reps, echo2use, TR_mte,esp_mte,TEs, t2s_vals, gap_between_readouts, time2relax_at_the_end, b1, inv_eff,pd_map )
%
...
delT_M2m_M3m = etl;                   % duration of readout#1
...
Mz_mtx = zeros([14, num_voxels, num_reps]);
Mxy_mtx = zeros([14, num_voxels, num_reps]);
...
%     M5 = -M4 .* inv_eff;
    ...
    Mxyecho = zeros(tf,num_voxels,10);
    time = 0;
    ...
    Mxy_mtx(:,:,reps) = cat(1, Mxy_acq0(echo2use,:), Mxy_acq1(echo2use,:), Mxy_acq2(echo2use,:), Mxy_acq3(echo2use,:), reshape(Mxyecho(end,:,1),[1,num_voxels]),reshape(Mxyecho(end,:,2),[1,num_voxels]),...
        ...
        ,reshape(Mxyecho(end,:,7),[1,num_voxels]),reshape(Mxyecho(end,:,8),[1,num_voxels]),reshape(Mxyecho(end,:,9),[1,num_voxels]),reshape(Mxyecho(end,:,10),[1,num_voxels]));
end
% toc
